<template>
    <div class="w-full">
        <!-- header -->
        <div v-for="(j, i) in userData" :key="i">
        ...
        <div class="flex gap-3 items-center justify-between w-full p-3 border-t border-gray-300">
            ...
            <input type="text" placeholder="Message"
                ...
                v-model="formChat.data"
                name="message" required />

            <button v-on:click="setChat()">
...
export default {
    ...
    computed: {
        chatData() { return store.chat; },
        userData() { return store.user; },
        chatId() { return store.chatId; }
    },
    mounted() {},
    methods: {
        setChat() {
            ...
            this.formChat.date = hour + ':' + minute;

            axios
                .patch('/api/chat/' + this.chatId, this.formChat)
                .then(res => { 
                    this.formChat.data = ''; 
                    mutations.setChat(this.chatId);
                });
        },
    }
}
